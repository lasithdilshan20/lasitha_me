...
export default function NPMPackageCard({
  ...
  return (
    <div className={`glass rounded-2xl p-5 border border-[color:var(--panel-border)] shadow-neon hover:shadow-glow transition ${featured ? 'ring-1 ring-cyber-blue/40' : ''}`}>
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="text-lg md:text-xl font-semibold text-cyber-blue">{name}</div>
          {version && <div className="text-xs text-neutral-500 dark:text-white/60">v{version}</div>}
        </div>
        {featured && <div className="text-2xl" aria-hidden>🏆</div>}
      </div>
      {description && <p className="mt-3 text-sm text-neutral-700 dark:text-white/80">{description}</p>}
      {typeof downloads === 'number' && (
        <div className="mt-4">
          <div className="text-xs text-neutral-500 dark:text-white/60">Total Downloads</div>
