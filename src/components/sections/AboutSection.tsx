...
export default function AboutSection() {
  ...
  return (
    <section id="about" aria-labelledby="about-heading">
      ...
      <p className="text-neutral-700 dark:text-white/80 max-w-3xl mb-8">Software Developer in Test specializing in Cypress automation. Passionate about quality engineering, observability, and developer experience. Crafting testable systems and sleek tooling with a cyberpunk aesthetic.</p>
      <div className="relative pl-6 border-l border-cyber-purple/40">
        {items.map((item, i) => (
          <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="mb-6">
            ...
            <div className="text-neutral-900 dark:text-white/90">{item.text}</div>
          </motion.div>
        ))}
      </div>
    </section>
